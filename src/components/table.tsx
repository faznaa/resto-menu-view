  ...
  export default function ItemsTable({ editClick, items, id }: any) {
    return (
      <div className="px-4 sm:px-6 lg:px-8">
       ...
       {items?.length > 0 ? ( <div className="mt-8 flow-root">
                ...
                <tbody className="divide-y divide-gray-200">
                  {items.map((item: Item) => (
                    <tr key={item._id}>
                      ...
                      <td className="truncate whitespace-nowrap px-3 py-4 text-sm text-gray-500 max-w-md">{item.description}</td>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                        <button onClick={() => editClick(item._id)} className="text-indigo-600 hover:text-indigo-900">
